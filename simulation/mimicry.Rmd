...
```{r}
nlobj@experiment@variables$mutation_rate
```

```{r}
# define prior on the mutation rate parameter
# define a model that given the mutation rate generates the summary statistics
# compute summary statistic on a real case, and pass in to the EasyABC function
```
```{r}
nlobj@experiment <- experiment(
  expname = "mimcry",
...
mutation_model <- function(nlobj) {
  function(rate) {
    ...
    color_stats <- results %>%
      dplyr::select(`[step]`, starts_with("count"), metrics.turtles) %>%
      unnest(metrics.turtles) %>%
      filter(breed != "birds") %>%
      group_by(breed, `[step]`) %>%
...
test_stat <- m(30)
ABC_mcmc(method = "Marjoram", model = m, prior = prior, summary_stat_target = test_stat)
```
