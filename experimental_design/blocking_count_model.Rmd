...
```{r}
simulate <- function(design, effects, depth = 100) {
...
fit <- model$variational(data = inputs)
```
...
```{r}
ggplot(treatment_estimates %>% filter(feature < 20)) +
  stat_interval(aes(y = as.factor(treatment), x = value)) +
  facet_wrap(~ feature)
```
