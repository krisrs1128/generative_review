...
x_blocked <- simulate(blocked_design, effects)
x_df <- bind_cols(blocked_design, x_blocked) %>%
...
```{r}
x_basic <- simulate(basic_design, effects)
x_df <- bind_cols(basic_design, x_basic) %>%
  pivot_longer(-c("id", "treatment", "subject"), names_to = "feature")
```
...
```{r}
model <- cmdstan_model("multinomial_logistic.stan")
inputs <- list(
  N = N,
  D = ncol(x_blocked),
  T = nrow(effects$treatment),
  S = n_distinct(blocked_design, "subject"),
  t_ix = as.integer(blocked_design$treatment),
  s_ix = blocked_design$subject,
  x = as.matrix(x_blocked)
)

fit <- model$sample(data = inputs, chains = 1)
```

```{r}
treatment_estimates <- tidy_draws(fit) %>%
 select(`.chain`, `.iteration`, starts_with("treatment")) %>%
